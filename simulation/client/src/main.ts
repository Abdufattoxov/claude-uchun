...
function onState(msg: StateMessage): void {
  clockEl.textContent = `Day ${msg.time.day}, ${pad(msg.time.hour)}:${pad(msg.time.minute)}`;
  weatherLabelEl.textContent = msg.weather;
...
function needBar(label: string, value: number): string {
  return `<div class="need-bar-row"><span class="label">${label}</span><div class="need-bar"><i style="width:${value}%"></i></div></div>`;
}
...
async function refreshInspector(): Promise<void> {
  ...
  inspectorContentEl.innerHTML = `
    <h2>${a.name}</h2>
    <div class="muted">${a.age} yrs &middot; ${a.occupation ?? "unemployed"} &middot; $${a.money.toFixed(2)}</div>
    <div class="muted">${a.currentActivity}</div>
    <h4>Needs</h4>
    ${Object.entries(a.needs).map(([k, v]) => needBar(k, v as number)).join("")}
    <h4>Emotion</h4>
    <div class="muted">${a.emotion.label} (valence ${a.emotion.valence.toFixed(2)})</div>
    <h4>Goals</h4>
    <ul>${a.goals.map((g) => `<li>${g.description} <span class="pill">${g.kind}</span></li>`).join("")}</ul>
    <h4>Relationships</h4>
    <ul>${detail.relationships.length ? detail.relationships.map((r) => `<li>${r.otherName}: ${r.state.replace("_", " ")} (${r.affinity.toFixed(0)})</li>`).join("") : '<li class="muted">No relationships yet</li>'}</ul>
    <h4>Recent memories</h4>
    <ul>${detail.memories.shortTerm.slice(0, 6).map((m) => `<li>${m.description}</li>`).join("") || '<li class="muted">Nothing yet</li>'}</ul>
    <h4>Recent decisions</h4>
    <ul>${detail.recentDecisions.slice(0, 5).map((d) => `<li>${d.action}: ${d.reason}</li>`).join("") || '<li class="muted">None yet</li>'}</ul>
  `;
}
...
eventSubmitBtn.addEventListener("click", async () => {
  ...
  eventSubmitBtn.textContent = "Injecting...";
  try {
    ...
    eventSubmitBtn.textContent = "Inject";
  }
});
...
async function pollEvents(): Promise<void> {
  ...
  for (const ev of events.slice().reverse()) {
    ...
    if (ev.kind === "admin_message" && (ev.payload as any).kind === "dialogue") {
      ...
      appendEventLog(`${p.speaker} to ${p.listener}: "${p.line}"`);
    } else if (ev.kind === "weather_change") {
      appendEventLog(`Weather shifted to ${(ev.payload as any).weather} (${(ev.payload as any).cause})`);
    } else if (ev.kind === "unknown_event") {
      appendEventLog(`Unknown event: ${(ev.payload as any).description}`);
    }
  }
